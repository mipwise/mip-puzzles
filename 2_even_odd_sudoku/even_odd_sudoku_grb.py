...
Created by Luiz Suzana and Aster Santana (Oct 2021), MipWise.com.
"""
...
for i in I:
    for j in I:
        mdl.addConstr(sum(x[(i, j, k)] for k in I) == 1, name=f'One-digit-per-cell-{(i,j)}')

# Digits can't repeat in each row
for i in I:
    for k in I:
        mdl.addConstr(sum(x[i, j, k] for j in I) == 1, name=f'No-repeat-row-{i}-value-{k}')

# Digits can't repeat in each column
for j in I:
    for k in I:
        mdl.addConstr(sum(x[i, j, k] for i in I) == 1, name=f'No-repeat-col-{j}-value-{k}')

# Digits can't repeat in each bold region
for region in BR:
    for k in I:
        mdl.addConstr(sum(x[i, j, k] for (i, j) in BR[region]) == 1, name=f'No-repeat-region-{region}-value-{k}')
...
for i, j in EC:
    mdl.addConstr(sum(x[i, j, k] for k in I if k % 2 == 0) == 1, name=f'Even-digit-cell-{(i, j)}')

# Some cells must have an odd digit
for i, j in OC:
    mdl.addConstr(sum(x[i, j, k] for k in I if k % 2 == 1) == 1, name=f'Odd-digit-cell-{(i, j)}')

# Set Objective function
mdl.setObjective(x[1, 1, 1], sense=gp.GRB.MAXIMIZE)  # we could choose any other non-constant function
# endregion
...
for i in I:
    print([int(sum(k * x[i, j, k].X for k in I)) for j in I])
# endregion
